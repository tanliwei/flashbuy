...
public class FlashbuyUserService {
    ...
    private void addCookie(HttpServletResponse response, String token, FlashbuyUser user) {
        redisService.set(FlashbuyUserKey.TOKEN, token, user);
        Cookie cookie = new Cookie(AUTH_TOKEN, token);
        cookie.setMaxAge(FlashbuyUserKey.TOKEN.expireSeconds());
        cookie.setPath("/");
    ...
    public FlashbuyUser getByToken(HttpServletResponse response, String token) {
        ...
        FlashbuyUser user = redisService.get(FlashbuyUserKey.TOKEN, token, FlashbuyUser.class);
